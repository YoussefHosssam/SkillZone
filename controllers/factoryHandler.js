...
const getAllDocuments = async ( Model, req , res , next)=>{
    ...
    const features = new docuemntFeatures(Model.find(),parsedQuery).filter().sort().paginate(req).fields()
...
const updateDocument = async (Model , body ,req , res , next)=>{
    ...
    if(document){
        successResponse(res , 200 , {message : "document updated successfully" , data : document})
    }
    else{
...
module.exports = {createDocument , getAllDocuments , getSingleDocument , deleteDocument , updateDocument}