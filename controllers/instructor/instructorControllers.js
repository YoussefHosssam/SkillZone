...
const getInstructor = asyncHandler(async (req, res, next) => {
    const {id} = req.params
    const instructorDetails = (await Course.findById(id)).instructorDetails
    if(instructorDetails){
        successResponse(res , 200 , {message : "Instructor details found successfully" , data : instructorDetails})
    }
    else{
...
const getBranchInstructors = asyncHandler(async (req, res, next) => {
    const {id} = req.params
    const instructors = (await Course.find({branchId : id}).select("+instructorDetails"))
    if(instructors){
        successResponse(res , 200 , {message : "Instructors found successfully" , data : instructors})
    }
    else{
...
const getInstructorCourses = asyncHandler(async (req, res, next) => {
    ...
    const courses = await Course.find({fullnameSlug : slug}.select("-instructorDetails"))
    if (courses){
        successResponse(res , 200 , {message : "Courses found successfully" , data : courses})
    }
    else{
