...
const {getSingleDocument , deleteDocument , updateDocument} = require(`${__dirname}/../factoryHandler.js`)
...
const updateBranch = asyncHandler (async (req , res , next)=>{
    const body = req.body
    const updateableFields = ["name" , "address" , "geoLocation" , "phone" , "operatingHours"]
...
const getNearbyBranches = asyncHandler(async (req, res, next) => {
    ...
    if (!lat || !lng || !radius) {
        return next(new AppError("Latitude, longitude, and radius are required.", 400));
    }
    ...
    if (!branches || branches.length === 0) {
        return next(new AppError("No nearby branches found for the provided coordinates.", 404));
    }
