...
const {generateTokens} = require(`${__dirname}/../../services/authServices.js`); 
...
const authUserRegister = asyncHandler(async(req , res , next)=>{
    ...
    await newUser.save()
    const verifyLink = `${req.protocol}://${req.get('host')}/api/v1/auth/verify-email/${token}`
    try{
    ...
    successResponse(res , 201 , {message : 'User created successfully. A verification email has been sent.' , data : newUser})
})
...
const authUserForgotPassword = asyncHandler(async(req , res , next)=>{
    ...
    if(user){
        try {
            ...
            sendEmail({from : process.env.EMAIL_MAIL , to : email , subject : "Reset Password recovery link" , message : `Your reset password link ${resetLink} `})
        }
        catch(err){
            user.resetPasswordToken = undefined 
            user.resetPasswordExpire = undefined
            await user.save({validateBeforeSave : false})
            return next(new AppError("Something went wrong while sending email please try again later" , 500))
        }
...
const authUserRefreshToken =asyncHandler(async(req , res , next)=>{
    ...
    if (valid){
        const newaccessToken = jwt.sign({id : decoded.id , role : decoded.role} , process.env.JWT_ACCESS_KEY , {expiresIn : process.env.JWT_ACCESS_EXPIRE })
...
module.exports = {authUserRegister , authUserLogin , authUserverifyEmail , authUserForgotPassword , authUserResetPassword , authUserRefreshToken , authUserlogOut} 